...
function renderBoard() {
    ...
    if (winningCells) {
        ...
        setTimeout(() => {
            document.querySelector('.turn-indicator').textContent = winner + "  has won!";
            previousButton.style.display = 'block';
            nextButton.style.display = 'block';
        }, 100);
    } else if (isBoardFull) {
        setTimeout(() => {
            document.querySelector('.turn-indicator').textContent = "The game ended in a tie. There is no winner.";
            previousButton.style.display = 'block';
            nextButton.style.display = 'block';
        }, 100);
    }
...
function renderMoveHistory() {
    ...
    moveHistory.forEach((move, index) => {
        ...
        historyItem.onclick = () => {
            ...
            const { player, row, col } = moveHistory[currentMoveIndex];
            board = moveHistory.slice(0, currentMoveIndex + 1).reduce((acc, move) => {
